...
def getTrailScore(topographicMap, cellStart):
    ...
    while (not (len(trailHeap) == 0)):
        cell = trailHeap.pop()
        if (topographicMap[cell[0]][cell[1]]==9 and (cell not in visitedTrailEnd)):
            visitedTrailEnd.append(cell)
            score += 1
        else:
...
assert(getTrailScore(topographicMap_test2, (5,5))==3)
assert(getTrailScore(topographicMap_test2, (5,2))==1)
assert(getTrailScore(topographicMap_test2, (0,2))==5)
assert(getTrailScore(topographicMap_test2, (0,4))==6)
assert(solve("day10_data_test2.txt")==36)
